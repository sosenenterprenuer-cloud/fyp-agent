...
def test_api_quiz_progressive_returns_10_questions(logged_in_client):
    ...
    # Check category distribution (15/15 split)
    category_counts = {}
    for q in questions:
        category = q['two_category']
        category_counts[category] = category_counts.get(category, 0) + 1
    
    assert category_counts.get('Data Modeling & DBMS Fundamentals', 0) == 15
    assert category_counts.get('Normalization & Dependencies', 0) == 15
