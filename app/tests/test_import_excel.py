...
def sample_questions_file():
    ...
    for i in range(12):
        data.append({
            ...
            'two_category': 'Data Modeling & DBMS Fundamentals',
    ...
    for i in range(12):
        data.append({
            ...
            'option_d': f'Option D{i+1}',
            'correct_answer': f'Option A{i+1}',
            'two_category': 'Normalization & Dependencies',
...
def test_import_questions(temp_db, sample_questions_file):
    ...
    cur = conn.execute("SELECT two_category, COUNT(*) FROM quiz GROUP BY two_category")
