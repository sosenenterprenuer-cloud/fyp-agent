...
def test_full_flow(tmp_path):
    ...
    with client as c:
      ...
      # verify 15/15 by two_category
      counts = {}
      for q in data['questions']:
          counts[q['two_category']] = counts.get(q['two_category'], 0) + 1
      assert counts.get('Data Modeling & DBMS Fundamentals') == 15
      assert counts.get('Normalization & Dependencies') == 15
